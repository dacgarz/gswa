...
class MStripe_ApiRequestor
{
  ...
  {
    if ($d instanceof MStripe_ApiResource) {
  ...
  public function handleApiError($rbody, $rcode, $resp)
  {
    if (!is_array($resp) || !isset($resp['error'])) {
      ...
      throw new MStripe_ApiError($msg, $rcode, $rbody, $resp);
    }
    ...
    case 400:
        if ($code == 'rate_limit') {
          throw new MStripe_RateLimitError(
    ...
    case 401:
        throw new MStripe_AuthenticationError($msg, $rcode, $rbody, $resp);
    case 402:
        throw new MStripe_CardError($msg, $param, $code, $rcode, $rbody, $resp);
    default:
        throw new MStripe_ApiError($msg, $rcode, $rbody, $resp);
    }
  }
  ...
  private function _requestRaw($method, $url, $params, $headers)
  {
    ...
    if (!$myApiKey) {
      $myApiKey = Migla_Stripe::$apiKey;
    }

    if (!$myApiKey) {
      $msg = 'No API key provided.  (HINT: set your API key using '
           . '"Migla_Stripe::setApiKey(<API-KEY>)".  You can generate API keys from '
           . 'the Stripe web interface.  See https://stripe.com/api for '
           . 'details, or email support@stripe.com if you have any questions.';
      throw new MStripe_AuthenticationError($msg);
    }
    ...
    $uname = php_uname();
    $ua = array(
        'bindings_version' => Migla_Stripe::VERSION,
        'lang' => 'php',
    ...
    $defaultHeaders = array(
        ...
        'User-Agent' => 'Stripe/v1 PhpBindings/' . Migla_Stripe::VERSION,
        'Authorization' => 'Bearer ' . $myApiKey,
    );

    if (Migla_Stripe::$apiVersion) {
      $defaultHeaders['Stripe-Version'] = Migla_Stripe::$apiVersion;
    }
  ...
  private function _interpretResponse($rbody, $rcode)
  {
    try {
      ...
      $msg = "Invalid response body from API: $rbody "
           . "(HTTP response code was $rcode)";
      throw new MStripe_ApiError($msg, $rcode, $rbody);
    }
  ...
  private function _curlRequest($method, $absUrl, $headers, $params, $hasFile)
  {
    ...
    if ($method == 'get') {
      ...
      throw new MStripe_ApiError("Unrecognized method $method");
    }
    ...
    if (!Migla_Stripe::$verifySslCerts)
  ...
  public function handleCurlError($errno, $message)
  {
    ...
    throw new MStripe_ApiConnectionError($msg);
  }
  ...
  private function checkSslCert($url)
  {
    ...
    if (($errno !== 0 && $errno !== NULL) || $result === false) {
      throw new MStripe_ApiConnectionError(
