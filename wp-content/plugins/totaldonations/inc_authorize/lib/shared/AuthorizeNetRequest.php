...
abstract class AuthorizeNetRequest
{
	...
	//addition by Astried
	if( defined('AUTHORIZENET_IS_VERIFY_PEER') ){
	   $this->VERIFY_PEER = AUTHORIZENET_IS_VERIFY_PEER;
	} 

        curl_setopt($curl_request, CURLOPT_SSL_VERIFYHOST, 2);
        if ($this->VERIFY_PEER) {
            curl_setopt($curl_request, CURLOPT_CAINFO, dirname(dirname(__FILE__)) . '/ssl/ca.pem');
        } else {
